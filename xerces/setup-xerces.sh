...
make -j$cores > "$logs_path/xerces_build_debug.log" 2>&1

# Install debug Xerces.
printf '\t- Installing debug Xerces...\n'
make install -j$cores > "$logs_path/xerces_install_debug.log" 2>&1
...
make -j$cores > "$logs_path/xerces_build_release.log" 2>&1

# Install release Xerces.
printf '\t- Installing release Xerces...\n'
make install -j$cores > "$logs_path/xerces_install_release.log" 2>&1
